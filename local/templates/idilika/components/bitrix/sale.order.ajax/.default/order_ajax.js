    ...
    BX.Sale.OrderAjaxComponent = {
        ...
        init: function (parameters) {
            ...
            if (this.totalBlockNode) {
                this.totalinfoBlockParentNode = this.totalBlockNode.querySelector('.bx-soa-cart-total-wrapper');
        ...
        editBasketItemsHeader: function (basketItemsNode) {
            ...
            for (i = 0; i < this.result.GRID.HEADERS.length; i++) {
                ...
                headers.push(
                    BX.create('DIV', {
                        props: {className: 'bx-soa-item-td bx-soa-item-properties '+ column.id + ' ' + (toRight ? ' bx-text-right' : '')},
                        style: {paddingBottom: '5px'},
                        children: [
                            BX.create('DIV', {
                                props: {className: 'bx-soa-item-td-title'},
                    ...
                    children: [
                        BX.create('DIV', {
                            ...
                            children: [
                                BX.create('INPUT', {
                                    props: {
                                        className: 'form-control bx-ios-fix owner_address',
                                        type: 'text'
                                    },
                                    events: {
        ...
        editFadeRegionContent: function (node) {
            ...
            if (locationProperty && locationString.length)
                addedHtml += BX.util.htmlspecialchars(locationString); // '<strong>' + BX.util.htmlspecialchars(locationProperty.NAME) + ':</strong> ' + BX.util.htmlspecialchars(locationString) + '<br>';
        ...
        getPersonTypeControl: function (node) {
            ...
            if (personTypesCount > 1) {
                input = BX.create('DIV', {
                    props: {className: 'form-group taxpayer'},
                    children: [
        ...
        createPaySystemItem: function (item) {
            ...
            label = BX.create('DIV', {
                ...
                children: [
                    ...
                    BX.create('LABEL', {
                        props: {className: 'delivery_label'},
                        html: ''
                    })
                ]
            ...
            if (this.params.SHOW_PAY_SYSTEM_LIST_NAMES == 'Y') {
                ...
                description = BX.create('DIV', {
                    props: {className: 'bx-soa-pp-company-smalldescription'},
                    text: this.params.SHOW_DELIVERY_PARENT_NAMES != 'N' ? item.DESCRIPTION : ''
                });
            }
        ...
        createDeliveryItem: function (item) {
            var checked = item.CHECKED == 'Y',
                ...
                logotype, label, title, description, itemNode, logoNode, price;
            ...
            itemNode = BX.create('DIV', {
                ...
                children: [label, title, description, price],
        ...
        editActivePropsBlock: function (activeNodeMode) {
            ...
            if (this.initialized.props) {
                ...
                propsNode = BX.create('DIV', {props: {className: 'row owner_fileds_row'}});
                selectedDelivery = this.getSelectedDelivery();
        ...
        editPropsItems: function (propsNode) {
            ...
            var propsItemsContainer = BX.create('DIV', {props: {className: 'col-sm-12 bx-soa-customer owner_fields flex'}}),
        ...
        alterProperty: function (settings, propContainer) {
            ...
            for (i = 0; i < inputs.length; i++) {
                inputs[i].placeholder = settings.NAME;
                BX.addClass(inputs[i], 'form-control bx-ios-fix owner_address');
            }
        ...
        totalBlockScrollCheck: function () {
            this.totalInfoBlockParentNode = this.totalBlockNode.querySelector('.bx-soa-cart-total-wrapper');
            if (!this.totalInfoBlockParentNode || !this.totalGhostBlockNode) {
                return false;
            }
            ...
            if ((windowHeight-footerHeight) > this.totalInfoBlockParentNode.offsetHeight) {

                if (ghostBottom - this.totalBlockNode.offsetHeight < scrollTop + 20)
                    BX.addClass(this.totalInfoBlockParentNode, 'bx-soa-cart-total-bottom');
                else
                    BX.removeClass(this.totalInfoBlockParentNode, 'bx-soa-cart-total-bottom');

                if (scrollTop > ghostTop && !BX.hasClass(this.totalInfoBlockParentNode, 'bx-soa-cart-total-fixed')) {
                    width = this.totalInfoBlockParentNode.offsetWidth;
                    BX.addClass(this.totalInfoBlockParentNode, 'bx-soa-cart-total-fixed');
                    this.totalGhostBlockNode.style.paddingTop = this.totalInfoBlockParentNode.offsetHeight + 'px';
                    this.totalInfoBlockParentNode.style.width = width + 'px';
                } else if (scrollTop < ghostTop && BX.hasClass(this.totalInfoBlockParentNode, 'bx-soa-cart-total-fixed')) {
                    BX.removeClass(this.totalInfoBlockParentNode, 'bx-soa-cart-total-fixed');
                    this.totalGhostBlockNode.style.paddingTop = 0;
                    this.totalInfoBlockParentNode.style.width = '';
                }
