				...
				foreach ( $arResult['ORDERS'] as $key => $order ) :
					...
					switch ( $order["ORDER"]["STATUS_ID"] ) {
						...
						case 'P':
							...
							break;
						case 'N':
							$status_order = "Обрабатывается";
							break;
					}

					?>
                    <tr>
                        <td data-title="№ заказа"><?= $order["ORDER"]["ID"] ?></td>
                        <td data-title="Дата заказа"><?= $order["ORDER"]["DATE_INSERT"] ?></td>
                        <td data-title="Сумма"><?= $order["ORDER"]["FORMATED_PRICE"] ?></td>
                        <td data-title="Статус"><?= $status_order ?></td>
                        <td><a href="<?= $order['ORDER']['URL_TO_DETAIL'] ?>">Подробнее</a></td>
                    </tr>
				<?endforeach; ?>
